...
const PlayerLobby: React.FC<PlayerLobbyProps> = ({ isHost }) => {
    ...
    const { currentPlayer, players, numberOfPlayer } = useAppSelector((state) => state.game);
    // const maxPlayers = players ? Math.max(4, players.length) : 4;
    // const spots = Array.from({ length: Math.min(maxPlayers, 8) }, (_, i) => i + 1);
    const maxSpots = players && players.length > 0 ? players.length : 0;
    const spotsArray = Array.from({ length: maxSpots }, (_, i) => i + 1)
    useEffect(() => {
    ...
    return (
        <>
            <h2 className="text-2xl font-bold text-white mb-2 text-center drop-shadow">
                {`${numberOfPlayer} người chơi đã tham gia phòng thi`}
            </h2>
            <div className={`grid ${spotsArray.length > 4 ? 'grid-cols-4 grid-rows-2' : 'grid-cols-4'} gap-6 mt-4 w-full`}>
                {spotsArray.map((spotNumber: number) => {
                    ...
                    if (player) {
                        return (
                            <div
                                key={spotNumber}
                                className={`flex items-center w-full min-h-[180px] bg-slate-800/80 rounded-xl p-4 shadow-md border border-slate-900 transition-all duration-200  ${isCurrent ? "ring-4 ring-yellow-400 border-yellow-400" : ""}`}
                            >
