...
const MCLobby = () => {
    const [mcUid, setMcUid] = useState<string>("")
    const { signInWithoutPassword, isAuthenticated, getUserInfo } = useAuth();
    const navigate = useNavigate();
    const [searchParams] = useSearchParams();
    const roomHostToken = searchParams.get("roomHostToken") || ""
    const [roomId, setRoomId] = useState<string>("")
    const [testName, setTestName] = useState<string>("")
    const [roomMode, setRoomMode] = useState<string>("")
    const {currentPlayer} = useAppSelector(state => state.game)

    useEffect (() => {
        if (roomHostToken) {
            const decoded: any = jwtDecode(roomHostToken);
            setRoomId(decoded["roomId"] || "")
            setTestName(decoded["testName"] || "")
            setRoomMode(decoded["roomMode"] || "")
        }
    }, [roomHostToken])
    ...
    useEffect(() => {
        ...
        const processUser = async () => {
            ...
            if (!userInfo) return; 
            console.log("MC Lobby User Info:", userInfo);
            // You can use userInfo to customize the lobby experience
            if (userInfo.hasOwnProperty("role") && userInfo["role"] === "MC") {
                navigate("/mc?roomId=" + userInfo["roomId"], { replace: true });
                return
            }

            setMcUid(currentPlayer?.uid || "")

        }

        processUser();
    }, [currentPlayer, roomId , testName ,roomMode]);
                ...
                <div className="bg-slate-800/80 backdrop-blur-sm border border-blue-400/30 rounded-xl shadow-2xl p-8 max-w-md w-full text-center">
                    ...
                    <div className="flex justify-center items-center space-x-4 mb-6">
                        ...
                        <div className="flex items-center space-x-2 text-blue-300">
                            ...
                            <span className="text-sm font-medium">Chờ host cấp quyền MC cho bạn</span>
                        </div>
                    </div>
